...
def get_citation_data_from_semantic_scholar(doi_start,target_folder):
# Fetch the API key from environment variables
 ...
 for paper_id in doi_list:
  ...
  for paper in data.get('data', []):
     ...
     citing_paper = paper.get('citingPaper', {})  # 获取引用论文的信息
     externalIds = citing_paper.get('externalIds', {})  # 使用 {} 作为默认值，防止 KeyError
     doi= '[]'
     authors = [author['name'] for author in citing_paper.get('authors', [])]  # 提取作者名单
     title = citing_paper.get('title', '{}')  # 提取标题
     year = citing_paper.get('year', '{}')  
     referenceCount = citing_paper.get('referenceCount', '{}')  
 ...
 output_file_name = os.path.join(target_folder, "citation_data.csv") 
